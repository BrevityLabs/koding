...
function getGoing(aggrType){
	...
	if(!isset(aggrType)) attrType = 'count';
	...
	//if(plotType == "full_year"){	//time and resource consuming animation. make animation smooth
	   ...
	   $.ajax({
			   type: 'GET',
			   url: '/koding/mapapp/index.php/sale/query',	//(controller/action)
			   data: {aggregate: aggrType},
			   dataType: 'json',
...
$(document).ready(function() {
	...
	$(":checkbox").change(showhidelabels($this));
	
}); //document.body.ready



function showhidelabels(elem){
	if($(elem).is(":checked")) {
