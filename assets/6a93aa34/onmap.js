...
// 	//Ajax call to the webservice
	$.ajax({
    	type: 'GET',
        url: 'monthrange.php',		//returns first and last dates in YYYY-MM-DD format
        dataType: 'json',
        success: function (result) {
            dmax = new Date(result.max);
            dmin = new Date(result.min);
            durantionInMonths = 12*(dmax.getFullYear()-dmin.getFullYear())+dmax.getMonth()-dmin.getMonth();	
        },
        complete: function (result) {
            var yr = dmin.getFullYear();
            	
            $("#pmonth").find('option').remove();

            for(var i=0,mth=dmin.getMonth(); i<durantionInMonths; i++,mth++) {
            	yr  = mth==12? yr++ : yr;								//mth is 0-indexed
            	mth = mth==12? 0 : mth;
				mt1 = mth<9? "0"+(mth+1): ""+(mth+1);
            	option1.append("<option value='" + yr + mt1 +"'>" + yr + "-" + mt1 + "</option>");
           		option2.append("<option value='" + yr + mt1 +"'>" + yr + "-" + mt1 + "</option>");
           		option3.append("<option value='" + yr + mt1 +"'>" + yr + "-" + mt1 + "</option>");
			}
		}
	});
 ...
 */
 function drawMap(labelsOn){

	 d3.json("//maps/counties.topojson", function(error, uk) {
		if (error)
			alert("may day, may day"); //return console.error(error);
	  	map_tag = uk;
	  	
		d3.json("uk.topojson", function(errorr, ukir){
			if(errorr)
